...
router.put('/reviews/:id', async (req, res) => {
    ...
    if (req.body.twitterhandle && req.body.textBody && req.body.rating) {
        try {
            ...
            } else res.status(404).json({ Error: `Review with ID ${id} does not exist.` });
        } catch (err) {
            res.status(500).json(err);
        }
        } else res.status(400).json({ Error: 'Please provide a twitterhandle, textBody & rating to edit review.' });
    });
    ...
    router.delete('/reviews/:id', async (req, res) => {
        ...
        count
            ? res.status(200).json({ message: `Review ${id} was successfully deleted.` })
            : res.status(404).json({ message: `ID: ${id} does not exist in our database` });
        } catch (err) {
        res.status(500).json(err);
        }
});
